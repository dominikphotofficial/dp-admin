...
} catch (e) { console.error("Main Firebase Init Error:", e); }
...
} catch (e) { console.error("Client Firebase Init Error:", e); }

const state = {
  currentView: 'dashboard',
  leads: [], galleries: [], portfolio: [],
  selectedLead: null,
  currentLang: 'lt',
...
authMain.onAuthStateChanged((user) => {
  ...
  if (user && user.email.toLowerCase() === window.CONFIG.ADMIN_EMAIL.toLowerCase()) {
    ...
    if (user) { authMain.signOut(); showToast("Prieiga uždrausta: netinkamas el. pašto adresas.", "error"); }
...
document.getElementById('btn-login-auth').addEventListener('click', async () => {
  ...
  if (email.toLowerCase() !== window.CONFIG.ADMIN_EMAIL.toLowerCase()) { showToast(`Prieiga leistina tik ${window.CONFIG.ADMIN_EMAIL}`, "error"); return; }
  try { await authMain.signInWithEmailAndPassword(email, pass); showToast("Sėkmingai prisijungta!", "success"); } 
  catch (err) { showToast("Autorizacijos klaida: " + err.message, "error"); }
});

document.getElementById('btn-logout').addEventListener('click', () => { authMain.signOut().then(() => showToast("Atsijungta", "info")); });

function showToast(msg, type = 'info') {
  const wrap = document.getElementById('toast-wrap');
  const toast = document.createElement('div');
  toast.className = `toast ${type}`;
  toast.innerHTML = `<i class="fa-solid ${type === 'success' ? 'fa-check' : 'fa-triangle-exclamation'}" style="margin-right: 8px;"></i> ${escapeHtml(msg)}`;
...
function switchView(viewName) {
  ...
  const titles = { dashboard: 'Apžvalga ir Rodikliai', leads: 'Užklausos & TFP Paraiškos', galleries: 'Klientų Portalas & Galerijos (API 2)', portfolio: 'Portfolio TVS (API 1)', settings: 'Nustatymai & Telegram Bot' };
...
function initDataListeners() {
  if (dbMain) {
    dbMain.collection('requests').orderBy('createdAt', 'desc').onSnapshot((snap) => {
      state.leads = []; snap.forEach(doc => state.leads.push({ id: doc.id, ...doc.data() })); renderLeads(); updateCounters();
    }, () => {
      dbMain.collection('tfp_requests').onSnapshot((snap) => { state.leads = []; snap.forEach(doc => state.leads.push({ id: doc.id, ...doc.data() })); renderLeads(); updateCounters(); });
    });
    dbMain.collection('portfolio').orderBy('createdAt', 'desc').onSnapshot((snap) => {
      state.portfolio = []; snap.forEach(doc => state.portfolio.push({ id: doc.id, ...doc.data() })); renderPortfolio(); updateCounters();
    });
  }
  if (dbClients) {
    dbClients.collection('galleries').orderBy('createdAt', 'desc').onSnapshot((snap) => {
      state.galleries = []; snap.forEach(doc => state.galleries.push({ id: doc.id, ...doc.data() })); renderGalleries(); updateCounters();
    });
  }
}
...
function updateCounters() {
  ...
  if (!state.leads.length) { recentBody.innerHTML = `<tr><td colspan="6"><div class="empty-placeholder">Naujų užklausų kol kas nėra</div></td></tr>`; return; }
...
function renderLeads() {
  ...
  const filtered = state.leads.filter(lead => {
    const name = (lead.name || '').toLowerCase(); const email = (lead.email || '').toLowerCase();
    const lang = (lead.language || lead.lang || 'LT').toUpperCase(); const status = lead.status || 'new';
    return (name.includes(search) || email.includes(search)) && (filterLang === 'all' || lang === filterLang) && (filterStatus === 'all' || status === filterStatus);
  });

  if (!filtered.length) { tbody.innerHTML = `<tr><td colspan="7"><div class="empty-placeholder">Užklausų nerasta</div></td></tr>`; return; }
...
function renderGalleries() {
  ...
  const filtered = state.galleries.filter(g => (g.clientName || '').toLowerCase().includes(search) || (g.token || '').toLowerCase().includes(search));
  if (!filtered.length) { grid.innerHTML = `<div style="grid-column: 1/-1;"><div class="empty-placeholder">Klientų galerijų nėra</div></div>`; return; }
...
function renderPortfolio() {
  ...
  const filtered = state.portfolio.filter(p => (p.title || '').toLowerCase().includes(search) || (p.category || '').toLowerCase().includes(search));
  if (!filtered.length) { grid.innerHTML = `<div style="grid-column: 1/-1;"><div class="empty-placeholder">Portfolio tuščias</div></div>`; return; }
...
function formatDate(ts) {
  if (!ts) return '-';
  try { const date = ts.toDate ? ts.toDate() : new Date(ts); return date.toLocaleDateString('lt-LT', { day: '2-digit', month: 'short', hour: '2-digit', minute: '2-digit' }); } 
  catch (e) { return '-'; }
}
...
function setTemplateLang(lang) { state.currentLang = lang; updateTemplatePreview(); }
...
function updateTemplatePreview() {
  ...
  const data = {
    ...
    status: document.getElementById('lead-modal-status-select').options[document.getElementById('lead-modal-status-select').selectedIndex].text
  };
...
document.getElementById('btn-lead-save-status').addEventListener('click', async () => {
  ...
  try {
    await dbMain.collection('requests').doc(state.selectedLead.id).update({ status: newStatus, updatedAt: firebase.firestore.FieldValue.serverTimestamp() });
    showToast("Būsena atnaujinta", "success"); closeModal('modal-lead');
  } catch (e) {
    try { await dbMain.collection('tfp_requests').doc(state.selectedLead.id).update({ status: newStatus, updatedAt: firebase.firestore.FieldValue.serverTimestamp() }); showToast("Būsena atnaujinta", "success"); closeModal('modal-lead'); } 
    catch (err) { showToast("Klaida išsaugant būseną", "error"); }
  }
});
...
document.getElementById('btn-lead-copy-html').addEventListener('click', () => {
  ...
  navigator.clipboard.writeText(html).then(() => showToast("HTML kodas nukopijuotas. Galite įklijuoti į Gmail.", "success"));
});
...
async function deleteLead(id) {
  ...
  try { await dbMain.collection('requests').doc(id).delete(); showToast("Užklausa ištrinta", "info"); } 
  catch (e) { try { await dbMain.collection('tfp_requests').doc(id).delete(); showToast("Užklausa ištrinta", "info"); } catch (err) { showToast("Klaida trinant", "error"); } }
}
...
document.getElementById('btn-open-new-gallery-modal').addEventListener('click', () => { generateGalleryToken(); openModal('modal-gallery'); });
...
document.getElementById('btn-save-gallery').addEventListener('click', async () => {
  ...
  if (!clientName || !token) { showToast("Užpildykite kliento vardą ir žetoną", "error"); return; }
  if (!dbClients) { showToast("API 2 neinicijuotas", "error"); return; }

  const saveBtn = document.getElementById('btn-save-gallery'); saveBtn.disabled = true;
  try {
    let coverUrl = '';
    if (fileInput.files.length > 0 && storageClients) {
      const file = fileInput.files[0];
      if (file.size > 5 * 1024 * 1024) { showToast("Failas per didelis (Maks. 5MB)", "error"); saveBtn.disabled = false; return; }
      const storageRef = storageClients.ref(`galleries/${token}/cover_${Date.now()}_${file.name}`);
      const uploadTask = storageRef.put(file);
      const pBox = document.getElementById('gal-progress-box'); const pBar = document.getElementById('gal-progress-bar');
      pBox.style.display = 'block';

      await new Promise((resolve, reject) => {
        uploadTask.on('state_changed', (snap) => { pBar.style.width = `${(snap.bytesTransferred / snap.totalBytes) * 100}%`; }, (err) => reject(err), async () => { coverUrl = await uploadTask.snapshot.ref.getDownloadURL(); resolve(); });
      });
    }

    await dbClients.collection('galleries').add({ clientName, type, shootDate, status, pin, token, coverUrl: coverUrl || 'https://images.unsplash.com/photo-1534528741775-53994a69daeb?auto=format&fit=crop&w=600&q=80', createdAt: firebase.firestore.FieldValue.serverTimestamp() });
    showToast(`Galerija klientui ${clientName} sukurta!`, "success"); closeModal('modal-gallery');
    document.getElementById('gal-client-name').value = ''; document.getElementById('gal-file-input').value = '';
  } catch (err) { showToast("Klaida: " + err.message, "error"); } 
  finally { saveBtn.disabled = false; document.getElementById('gal-progress-box').style.display = 'none'; }
});

function copyGalleryLink(token) { navigator.clipboard.writeText(`https://clients.dominikphotofficial.lt/#/gallery/${token}`).then(() => showToast("Nuoroda nukopijuota!", "success")); }
async function deleteGallery(id) { if (!confirm("Ištrinti galeriją?")) return; try { await dbClients.collection('galleries').doc(id).delete(); showToast("Galerija ištrinta", "info"); } catch (e) { showToast("Klaida trinant", "error"); } }

document.getElementById('btn-open-new-portfolio-modal').addEventListener('click', () => openModal('modal-portfolio'));
...
document.getElementById('btn-save-portfolio').addEventListener('click', async () => {
  ...
  if (!title) { showToast("Nurodykite pavadinimą", "error"); return; }
  if (!dbMain) { showToast("API 1 neinicijuotas", "error"); return; }

  const saveBtn = document.getElementById('btn-save-portfolio'); saveBtn.disabled = true;
  try {
    let coverUrl = '';
    if (fileInput.files.length > 0 && storageMain) {
      ...
      const uploadTask = storageRef.put(file);
      const pBox = document.getElementById('port-progress-box'); const pBar = document.getElementById('port-progress-bar');
      pBox.style.display = 'block';
    ...
    await dbMain.collection('portfolio').add({ title, category, description, coverUrl: coverUrl || 'https://images.unsplash.com/photo-1517841905240-472988babdf9?auto=format&fit=crop&w=600&q=80', createdAt: firebase.firestore.FieldValue.serverTimestamp() });
    showToast("Projektas publikuotas", "success"); closeModal('modal-portfolio');
    document.getElementById('port-title').value = ''; document.getElementById('port-desc').value = ''; document.getElementById('port-file-input').value = '';
  } catch (e) { showToast("Klaida: " + e.message, "error"); } 
  finally { saveBtn.disabled = false; document.getElementById('port-progress-box').style.display = 'none'; }
});

async function deletePortfolioItem(id) { if (!confirm("Ištrinti projektą?")) return; try { await dbMain.collection('portfolio').doc(id).delete(); showToast("Projektas ištrintas", "info"); } catch (e) { showToast("Klaida trinant", "error"); } }

async function sendTelegramMessage(text) {
  const token = state.tgToken; const chatId = state.tgChatId;
  if (!token || !chatId) { showToast("Nurodykite Bot Token ir Chat ID nustatymuose", "error"); return false; }
  try {
    const res = await fetch(`https://api.telegram.org/bot${token}/sendMessage`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ chat_id: chatId, text: text, parse_mode: 'Markdown' }) });
    const data = await res.json();
    if (data.ok) { showToast("Išsiųsta į Telegram!", "success"); return true; } 
    else { showToast(`TG Klaida: ${data.description}`, "error"); return false; }
  } catch (e) { showToast("Tinklo klaida", "error"); return false; }
}
...
document.getElementById('btn-save-settings').addEventListener('click', () => {
  const t = document.getElementById('setting-tg-token').value.trim(); const c = document.getElementById('setting-tg-chat-id').value.trim();
  state.tgToken = t; state.tgChatId = c; localStorage.setItem('dp_admin_tg_token', t); localStorage.setItem('dp_admin_tg_chat_id', c);
  showToast("Nustatymai išsaugoti", "success");
});
document.getElementById('btn-test-telegram').addEventListener('click', () => sendTelegramMessage("⚡️ *DP ADMIN:* Testinis pranešimas sėkmingai gautas!"));

['search-leads', 'filter-lead-lang', 'filter-lead-status'].forEach(id => document.getElementById(id).addEventListener('input', renderLeads));
